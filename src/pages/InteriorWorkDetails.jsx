...
function InteriorWorkDetails() {
  const { id } = useParams();
  const [interiorWork, setInteriorWork] = useState(null);
  const [similarInteriorWorks, setSimilarInteriorWorks] = useState([]);
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: interiorWork.title,
        text: `Check out this interior-work: ${interiorWork.title}`,
  ...
  if (!interiorWork) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center bg-gray-50 pt-24 px-6 text-center">
        <h2 className="text-4xl font-serif text-gray-900 mb-4">InteriorWork Not Found</h2>
        <p className="text-gray-500 mb-8">The listing you are looking for may have been removed or is currently unavailable.</p>
        <Link to="/interior-works" className="px-8 py-3 bg-primary text-white rounded-full font-medium hover:shadow-lg transition-all">
          Browse Portfolio
        </Link>
      </div>
    );
  ...
  let parsedAmenities = [];
  if (interiorWork.amenities) {
    try { parsedAmenities = typeof interiorWork.amenities === 'string' ? JSON.parse(interiorWork.amenities) : interiorWork.amenities; } catch(e){}
  }
  
  let parsedNearby = [];
  if (interiorWork.nearby_places) {
    try { parsedNearby = typeof interiorWork.nearby_places === 'string' ? JSON.parse(interiorWork.nearby_places) : interiorWork.nearby_places; } catch(e){}
  }

  const allImages = interiorWork.images 
    ? (typeof interiorWork.images === 'string' ? JSON.parse(interiorWork.images) : interiorWork.images)
    : [interiorWork.image];
    ...
    <div className="bg-gray-50 min-h-screen font-sans selection:bg-primary selection:text-white pb-20">
      ...
      <div className="relative w-full h-[60vh] min-h-[500px] bg-dark pt-20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 h-full flex flex-col justify-end pb-12 pointer-events-none">
          <Link to="/interior-works" className="inline-flex items-center gap-2 text-white/80 hover:text-white transition-colors w-fit mb-6 bg-black/20 backdrop-blur-md px-4 py-2 rounded-full text-sm font-medium border border-white/10 pointer-events-auto shadow-lg hover:bg-black/40">
            ...
            <div className="max-w-3xl">
              <div className="flex gap-3 mb-4">
                <span className="bg-primary text-white px-4 py-1.5 rounded-full text-xs font-bold tracking-wider uppercase shadow-lg">
                  {interiorWork.interior_type || 'For Sale'}
                </span>
              ...
              <div className="flex items-center gap-2 text-white/90 text-xl font-medium drop-shadow-md">
                <MapPin size={24} className="text-primary" /> {interiorWork.address}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 xl:gap-12">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            >
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 pb-10 border-b border-gray-100 mb-10">
                {interiorWork.interior_type === "For Rent" ? (
                  <div className="flex flex-wrap gap-12">
                    <div>
                      ...
                      <p className="text-4xl md:text-6xl font-serif font-bold text-gray-900">
                        ₹{interiorWork.rent_amount?.toLocaleString()}
                      </p>
                    </div>
                    <div className="border-l border-gray-100 pl-12">
                      ...
                      <p className="text-4xl md:text-6xl font-serif font-bold text-gray-900">
                        ₹{interiorWork.deposit_amount?.toLocaleString()}
                      </p>
                    </div>
                  </div>
                ) : (
                  <div>
                    ...
                    <div className="flex items-baseline gap-3">
                      <p className="text-4xl md:text-6xl font-serif font-bold text-gray-900">
                        ₹{((interiorWork.offer_price || interiorWork.price) / 100000).toFixed(2)}
                      </p>
                      <span className="text-3xl text-gray-400 font-serif">Cr</span>
                    </div>
                    {interiorWork.offer_price && interiorWork.price > interiorWork.offer_price && (
                      <div className="mt-4 flex items-center gap-3">
                        <span className="line-through text-gray-400 text-lg">₹{(interiorWork.price / 100000).toFixed(2)} Cr</span> 
                        <span className="bg-green-100 text-green-700 px-3 py-1 rounded-full text-sm font-bold">
                          Save ₹{((interiorWork.price - interiorWork.offer_price) / 100000).toFixed(2)} Cr
                        </span>
                      </div>
                    )}
                  </div>
              ...
              {/* KEY DETAILS GRID */}
              {(interiorWork.area || interiorWork.bedrooms || interiorWork.bathrooms || interiorWork.parking) && (
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-12">
                  {[
                    { icon: Maximize2, value: interiorWork.area, label: "Sq Ft" },
                    { icon: Bed, value: interiorWork.bedrooms, label: "Bedrooms" },
                    { icon: Bath, value: interiorWork.bathrooms, label: "Bathrooms" },
                    { icon: Car, value: interiorWork.parking, label: "Parking" }
              ...
              {interiorWork.map_location && (
                <div>
                  ...
                  <div className="w-full h-[500px] rounded-[2rem] overflow-hidden border border-gray-100 shadow-2xl">
                    {interiorWork.map_location.includes('<iframe') ? (
                      <div className="w-full h-full" dangerouslySetInnerHTML={{ __html: interiorWork.map_location.replace(/width="[^"]+"/, 'width="100%"').replace(/height="[^"]+"/, 'height="100%"') }} />
                    ) : (
                      <iframe 
                        src={interiorWork.map_location} 
                        width="100%" 
                        height="100%" 
                        style={{ border: 0 }} 
                        allowFullScreen="" 
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-32 space-y-8">
                ...
                <div className="relative z-10">
                  ...
                  <div className="mt-10 pt-10 border-t border-gray-100">
                    <div className="flex items-center gap-5">
                      ...
                      <div>
                        <p className="text-sm text-gray-500 font-bold uppercase tracking-wider">InteriorWork Consultant</p>
              ...
              >
                <Share size={20} /> Share InteriorWork
              </button>

              {/* INQUIRY FORM */}
              <InquiryForm interiorWorkId={interiorWork.id} interiorWorkTitle={interiorWork.title} />
            </div>
          </div>
        ...
        {similarInteriorWorks.length > 0 && (
          <div className="mt-24">
            <div className="flex justify-between items-end mb-12">
              ...
              <Link to="/interior-works" className="text-primary font-bold flex items-center gap-2 hover:gap-3 transition-all mb-2">
                View All Portfolio <ArrowRight size={20} />
              </Link>
            </div>
            
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
              {similarInteriorWorks.map((p, idx) => (
                <InteriorWorkCard key={p.id} interiorWork={p} idx={idx} />
              ))}
            </div>
          </div>
        )}
...
export default InteriorWorkDetails;
